...
2. **auggie 必需**: 必须安装并配置 auggie 作为代码搜索工具
3. **智能分流**: Phase 1 后自动判断任务类型
4. **Phase 3 不可跳过**: 必须澄清所有疑问
5. **Phase 5 TDD 强制**: 必须先写测试
6. **并行优先**: 能并行的任务必须并行执行
7. **状态持久化**: 每个阶段后自动更新状态文件（无需确认）
8. **选项式编程**: 使用 AskUserQuestion 展示选项
9. **文档更新询问**: Phase 7 必须询问
...
**重要**: auggie 是工作流的**必需依赖**，提供高级代码上下文搜索：
